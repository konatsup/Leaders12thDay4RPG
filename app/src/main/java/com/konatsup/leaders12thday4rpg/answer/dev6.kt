...
// Monsterクラスを継承したDragonクラス(hp: 50, attackPoint: 20, defensePoint: 10)
class Dragon(override val name: String) : Monster() {
    ...
    // fireメソッドの追加
    fun fire(target: Hero) {
        target.hp = target.hp - attackPoint * 2
    }
}

// Monsterクラスを継承したOgreクラス(hp: 50, attackPoint: 10, defensePoint: 20)
